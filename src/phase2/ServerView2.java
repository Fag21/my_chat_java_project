...
public class ServerView2 {
	 ...
	 * https://examples.javacodegeeks.com/enterprise-java/jms/apache-activemq-hello-
	 * world-example/
	 * https://stackoverflow.com/questions/11839246/how-to-convert-timestamp-to-date
	 * -in-java
	 */
	...
	private static Map<String, Socket> allUsersList = new ConcurrentHashMap<>(); // keeps the mapping of all the
																					// usernames used and their socket
																					// connections
	private static Set<String> activeUserSet = new HashSet<>(); // this set keeps track of all the active users

	private static Map<String, MessageProducer> userWiseMessageProducerMap = new ConcurrentHashMap<>(); // map to keep
																										// ActiveMQ
																										// producer for
																										// every client

	private static Map<String, MessageConsumer> userWiseMessageConsumerMap = new ConcurrentHashMap<>();// map to keep
																										// ActiveMQ
																										// consumer for
																										// every client
	...
	private DefaultListModel<String> activeDlm = new DefaultListModel<String>(); // keeps list of active users for
																					// display on UI

	private DefaultListModel<String> allDlm = new DefaultListModel<String>(); // keeps list of all users for display on
																				// UI
		...
		public void run() {
			while (true) {
				try {
					...
					String uName = new DataInputStream(clientSocket.getInputStream()).readUTF(); // this will receive
																									// the username sent
																									// from client
																									// register view
					DataOutputStream cOutStream = new DataOutputStream(clientSocket.getOutputStream()); // create an
																										// output stream
																										// for client
					if (activeUserSet != null && activeUserSet.contains(uName)) { // if username is in use then we need
																					// to prompt user to enter new name
						cOutStream.writeUTF("Username already taken");
					} else {
						// first we will register a new messagig queue on ActiveMQ
						ConnectionFactory connectionFactory = new ActiveMQConnectionFactory(url); // make connection
																									// factory
						...
						Session session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE); // Create session
																										...
																										// receive
																										// messages.
						Destination destination = session.createQueue(uName);// destination queue where msg will be
																				// stored
						MessageProducer producer = null;
						if (!userWiseMessageProducerMap.containsKey(uName)) { // if producer already created then skip
																				// creating again
							producer = session.createProducer(destination);// producer is used for sending messages to
																			// the queue.
							userWiseMessageProducerMap.put(uName, producer); // add producer to the map. so that only 1
																				// producer per client is made
						}
						MessageConsumer consumer = null;
						if (!userWiseMessageConsumerMap.containsKey(uName)) {// if consumer is already made then skip
																				// creating again
							consumer = session.createConsumer(destination); // create consumer.
							userWiseMessageConsumerMap.put(uName, consumer); // add consumer to the map for future use.
																				// this will make sure that only 1
																				// consumer per client exist
						}
						...
						if (!allDlm.contains(uName)) // if username taken previously then don't add to allUser JList
														// otherwise add it
							allDlm.addElement(uName);
						activeClientList.setModel(activeDlm); // show the active and allUser List to the swing app in
																// JList
						allUserNameList.setModel(allDlm);
						serverMessageBoard.append("Client " + uName + " Connected...\n"); // print message on server
																							// that new client has been
																							// connected.
	...
	class MsgRead extends Thread { // this class reads the messages coming from client and take appropriate actions
		...
		public void run() {
			...
			while (allUserNameList != null && !allUsersList.isEmpty()) { // if allUserList is not empty then proceed
																			// further
				try {
					...
					String[] msgList = message.split(":"); // I have used my own identifier to identify what action to
															// take on the received message from client
															// i have appended
															// actionToBeTaken:clients_for_receiving_msg:message
					if (msgList[0].equalsIgnoreCase("multicast")) { // if action is multicast then send messages to
																	// selected active users
						String[] sendToList = msgList[1].split(","); // this variable contains list of clients which
						...
						for (String usr : sendToList) { // for every user send message
							try {
								...
								if (userWiseMessageProducerMap.containsKey(usr)) { // if producer already exist then use
																					// it otherwise create new producer
																					// and add to map
								...
								if (activeUserSet.contains(usr)) { // check again if user is active then send the
																	// message
									// we need to make sure that there must be only one consumer for a queue
									// otherwise it will freeze as new consumer will wait for messages while
									// messages were in older consumer
									...
									String timestamp = new java.text.SimpleDateFormat("MM/dd/yyyy HH:mm:ss")
											.format(new java.util.Date(receivedMsg.getJMSTimestamp()));
									if (receivedMsg instanceof TextMessage) {
										TextMessage textMessage = (TextMessage) receivedMsg; // get the message from the
																								// queue
					...
					} else if (msgList[0].equalsIgnoreCase("broadcast")) { // if broadcast then send message to all
																			// active clients
						...
						while (itr1.hasNext()) {
							...
							if (userWiseMessageProducerMap.containsKey(usrName)) { // use the existing producer for the
																					// client
							...
							if (!usrName.equalsIgnoreCase(Id)) { // we don't need to send message to ourself, so we
																	// check for our Id
								try {
									if (activeUserSet.contains(usrName)) { // if client is active then send message
					...
					} else if (msgList[0].equalsIgnoreCase("exit")) { // if a client's process is killed then notify
																		// other clients
							...
							if (!usrName2.equalsIgnoreCase(Id)) { // we don't need to send this message to ourself
								try {
									new DataOutputStream(((Socket) allUsersList.get(usrName2)).getOutputStream())
											.writeUTF(Id + " disconnected...\n"); // notify all other active user for
																					// disconnection of a user
								} catch (Exception e) { // throw errors
									e.printStackTrace();
								}
								new PrepareCLientList().start(); // update the active user list for every client after a
					...
					} else if (msgList[0].equalsIgnoreCase("checkmessage")) { // here we will retrieve messages from the
																				// queue posted when user was offline
						...
						try {
							...
							connection.start();
							// Create session to send or receive messages.
							Session session = connection.createSession(false, Session.AUTO_ACKNOWLEDGE); // destination
																											// queue
																											// where msg
																											// will be
																											// stored
							// use the existing consumer as we want only 1 consumer per queue otherwise
							// there will be problem
							MessageConsumer consumer = userWiseMessageConsumerMap.get(id);
							// there is no method to count messages from the queue so we will use enumerator
							// to retrieve all the messages one by one.
							Queue queue = session.createQueue(id); // create queue
							QueueBrowser qb = session.createBrowser(queue); // create queuebrowser for using enumerator
																			// upon
							...
							while (enumeration.hasMoreElements()) { // loop on all the messages available in queue
								Message m = (Message) enumeration.nextElement(); // get message without removing it from
																					// queue.
								String timestamp = new java.text.SimpleDateFormat("MM/dd/yyyy HH:mm:ss")
										.format(new java.util.Date(m.getJMSTimestamp()));
								System.out.println("received messsagessss:" + ((TextMessage) m).getText());
								if (outputMsg == "")
									outputMsg += ((TextMessage) m).getText() + "\n" + timestamp + "\n";
								else
									outputMsg += "\n" + ((TextMessage) m).getText() + "\n" + timestamp + "\n";
								// enumerator show the message without deleting it from queue but we need to
								// remove it so will use consumer.receive to remove it.
				...
				while (itr.hasNext()) { // iterate over all active users
					...
					try {
						new DataOutputStream(((Socket) allUsersList.get(key)).getOutputStream())
								.writeUTF(":;.,/=" + ids); // set output stream and send the list of active users with
															// identifier prefix :;.,/=
					} catch (Exception e) {
						e.printStackTrace();
					}
