...
public class LoginClient2 extends JFrame {
	...
	public static void main(String[] args) { // main function which will make UI visible
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					LoginClient2 window = new LoginClient2();
		...
		clientLoginBtn.addActionListener(new ActionListener() { // action will be taken on clicking login button
			public void actionPerformed(ActionEvent e) {
				try {
					...
					DataInputStream inputStream = new DataInputStream(s.getInputStream()); // create input and output
																							// stream
					DataOutputStream outStream = new DataOutputStream(s.getOutputStream());
					outStream.writeUTF(id); // send username to the output stream

					String msgFromServer = new DataInputStream(s.getInputStream()).readUTF(); // receive message on
																								// socket
					if (msgFromServer.equals("Username already taken")) {// if server sent this message then prompt user
																			// to enter other username
						JOptionPane.showMessageDialog(frame, "Username already taken\n"); // show message in other
																							// dialog box
					} else {
						new ClientView2(id, s); // otherwise just create a new thread of Client view and close the
												// register jframe
						frame.dispose();
					}
				} catch (Exception ex) {
