        ...
        public void run() {
            while (true) {
                try {
                    ...
                    } else if (activeUserSet != null && activeUserSet.contains(uName)) { // if username is in use then
                                                                                         // we need to prompt user to
                                                                                         // enter new name
                ...
                try {
                    ...
                    if (msgList[0].equalsIgnoreCase("multicast")) { // if action is multicast then send messages to
                        ...
                        if (!vClock.containsKey(from)) { // condition to select which clock to update.
                            ...
                            vClock.put(from, val);
                        } else { // if message received is not for the first time then update the respective
                                 // clocks by 1
                        ...
                        if (!vClock.containsKey(to)) { // similarly we will update clock of receiver client too.
                            int[] val = { 0, 0, 0 };
                            if (to.equalsIgnoreCase("a")) { // this is the case if message is being sent for the first
                                                            // time.
                                val[0] = 1;
                            ...
                            if (to.equalsIgnoreCase("a")) { // this is the case if message recieved is not for the first
                                                            // time.
                                vClock.get(to)[0] += 1;
                                vClock.get(to)[1] = Math.max(vClock.get(from)[1], vClock.get(to)[1]);// compare the time
                                                                                                     // and update the
                                                                                                     // clock
                                vClock.get(to)[2] = Math.max(vClock.get(from)[2], vClock.get(to)[2]);// compare the time
                                                                                                     // and update the
                                                                                                     // clock
                            } else if (to.equalsIgnoreCase("b")) {
                                vClock.get(to)[0] = Math.max(vClock.get(from)[0], vClock.get(to)[0]);// compare the time
                                                                                                     // and update the
                                                                                                     // clock
                                vClock.get(to)[1] += 1;
                                vClock.get(to)[2] = Math.max(vClock.get(from)[2], vClock.get(to)[2]);// compare the time
                        ...
                        new DataOutputStream(((Socket) allUsersList.get(from)).getOutputStream())
                                .writeUTF("<Msg sent to: " + to + " >" + Arrays.toString(vClock.get(from))); // put
                                                                                                             // message
                                                                                                             // in
                                                                                                             // output
                                                                                                             // stream
                        new DataOutputStream(((Socket) allUsersList.get(to)).getOutputStream())
                                .writeUTF("< " + from + " >" + Arrays.toString(vClock.get(to))); // put message in
                                                                                                 // output stream
                        serverMessageBoard.append(from + ":" + Arrays.toString(vClock.get(from)) + "  ,  " + to + ":"
                                + Arrays.toString(vClock.get(to)) + "\n");
                            ...
                            if (!usrName2.equalsIgnoreCase(Id)) { // we don't need to send this message to ourself
                                try {
                                    new DataOutputStream(((Socket) allUsersList.get(usrName2)).getOutputStream())
                                            .writeUTF(Id + " disconnected..."); // notify all other active user for
